...
D_LONGSimulate = 0  # int(LONGSimulate)
#----------------------------------------------------------
d_TopLast = int(0)
D_TopBidPrice = int(0)
d_BottomPrice = int(0)
...
def Ttrading_Stop(ws, message, Symbol):
   ...
   #------------------------------------------------------
    ...
    #------------------------------------------------------
    # except for Simurate
    if(ws is not None and message is not None and D_LONGSimulate == 0):
        # Errorで発行できなかった
        ...
        # else --------------------------------------------
        print("%d ----- 返済売りマスク -----", D_LONGSimulate)
    ...
    # T_Now = datetime.datetime.now().time()
    # s_Time = T_Now.strftime("%H:%M:%S:%f")
    # s_dsp = "Ttrading_Stop(E ,%s),%s,%s" % (Symbol, S_SellTime, s_Time)
...
def LongOrder(ws, message, Symbol):
    ...
    pprint.pprint(json_data)
    s_dsp = "801 LONGSimulate:%d,%s,%s,,,%d,----- Long Order json_data -----" % (D_LONGSimulate, Symbol, S_CurrentTime, D_BidPrice)
    print(s_dsp)
    if(F_Write):
        WriteCsvFile.write(s_dsp + '\n')
...
def Long_Symbol1(ws, message, Symbol, WriteCsvFile):
    ...
    # for simirate
    # s_dsp = "000-0 Long_Symbol1,%s" % (Symbol)
    # print(s_dsp)
    # if(F_Write):
    #     WriteCsvFile.write(s_dsp + '\n')
    # s_dsp = "000-1 Long_Symbol1,%s" % (Symbol)
    # print(s_dsp)
    #------------------------------------------------------
    ...
    d_short_selling = conf['4thQuadrant']['short_selling']
    ...
    D_LONGLOSS = int(LONGLOSS)
    D_LONGCLOSE = int(LONGCLOSE)
    D_LONGSimulate = int(LONGSimulate)
    ...
    try:
        ...
        short_selling           = config['OrderEvent']['short_selling']
        pause_time               = config['kabuAPI']['pause_time']
        ...
        s_dsp = "100 Event driven   buy:%d  L_buy:%d  H_short:%d  short:%d" % (d_buy_order, d_lowest_buy_order, d_highest_short_selling, d_short_selling)
        print(s_dsp)
        if(F_Write):
            WriteCsvFile.write(s_dsp + '\n')
        # Event値は'0'に戻す
        if(buy_order != '0' or lowest_buy_order != '0' or highest_short_selling != '0' or short_selling != '0' or pause_time != '0'):
            config['OrderEvent']['buy_order'] = '0'
            config['OrderEvent']['lowest_buy_order'] = '0'
            config['OrderEvent']['highest_short_selling'] = '0'
            config['OrderEvent']['short_selling'] = '0'
        
            config['kabuAPI']['pause_time'] = '0'
    ...
    if (pause_time != '0'):
        ...
        if (WriteCsvFile):
            ...
            # config.add_section('OnMessage')
            # config.set('OnMessage', 'PauseTime', '0000')
            # # Writing our configuration file to '.ini'
            # with open('./TradeSymbol.ini', 'w') as configfile:
            #     config.write(configfile)       
     
    # if(D_CancelSt == 1 and S_BidOrderId is not None):
    #     s_dsp = "003 + CANCEL %s, %s, cancel_position(), (LONG0:%d, L_1st:%d, L_2nd:%d,) " % (s_Time,s_BidTime, D_LONG0, D_LONG1st, D_LONG2nd)
    #     print(s_dsp)
    #     if(F_Write):
    #         WriteCsvFile.write(s_dsp + '\n')

    #     D_CancelSt = kabu_CancelOrder.APICancel.cancel_position(Password=Password, OrderID=S_BidOrderId, Token=Token)
    #     s_dsp = "003 + CANCEL %s, %s, %d=cancel_position(), (LONG0:%d, L_1st:%d, L_2nd:%d,) " % (s_Time,s_BidTime, D_CancelSt, D_LONG0, D_LONG1st, D_LONG2nd)
    #     print(s_dsp)
    #     if(F_Write):
    #         WriteCsvFile.write(s_dsp + '\n')
    ...
    if(Symbol == Issue1 and Symbol is not None):
        ...
        s_dsp = "111,%s,,%s,%d,%d,   %d,%d,   %d,%d,%d,   %d,%d" % (Symbol, s_Time,D_Current, d_currentDiff, D_BidPrice, d_profit, d_TopLast, D_TopBidPrice,  d_BottomPrice, d_LosscutPrice, D_TargetPrice)
        if(D_LogLevel2 > 0):
            print(s_dsp)
        ...
        # s_dsp = "120,%s,,%s,%d,%d,   %d,%d,   %d,%d,%d,   %d,%d" % (Symbol, s_Time,D_Current, d_currentDiff, D_BidPrice, d_profit, d_TopLast, D_TopBidPrice,  d_BottomPrice, d_LosscutPrice, D_TargetPrice)
        # print(s_dsp)
        # if(F_Write):
        #     WriteCsvFile.write(s_dsp + '\n')
        # status = Symbol_Price(ws, message, Symbol, WriteCsvFile)
        # s_dsp = "121 %d=Symbol_Price(),%s,,%s,%d,%d,   %d,%d,   %d,%d,%d,   %d,%d" % (status, Symbol, s_Time,D_Current, d_currentDiff, D_BidPrice, d_profit, d_TopLast, D_TopBidPrice,  d_BottomPrice, d_LosscutPrice, D_TargetPrice)
        # print(s_dsp)
        # if(F_Write):
        #     WriteCsvFile.write(s_dsp + '\n')
        #-------------------------------------------------------
        ...
        # d_currentDiff = 0
        # if(D_CurrentLast != D_Current):
        #     d_currentDiff = D_CurrentLast - D_Current

        # s_dsp = "000-4 Long_Symbol1,%s" % (Symbol)
        # print(s_dsp)

        # s_dsp = "--100--,%s,,%s,%d,%d,   %d,%d,   %d,%d,%d,   %d,%d" % (Symbol, s_Time, D_Current, d_currentDiff, D_BidPrice, d_profit, d_TopLast, D_TopBidPrice,  d_BottomPrice, d_LosscutPrice, D_TargetPrice)
        # print(s_dsp)
        # if(F_Write):
        #     WriteCsvFile.write(s_dsp + '\n')
        #----------------------------------------------------------
        ...
        if(D_BidPrice == 0):
            ...
            s_dsp = "DIFF:%d,%s,,---,%d,(Top:%d - Bottom:%d), L_0:%d, L_1st:%d, L_2nd:%d, L_Loss:%d, L_3rd:%f, L_4th:%d" % (d_topBottom, Symbol, D_Current, D_TopBidPrice , d_BottomPrice,D_LONG0, D_LONG1st, D_LONG2nd, D_LONGLOSS, F_LONG3rd, D_LONG4th)
            print(s_dsp)
            if(F_Write):
                WriteCsvFile.write(s_dsp + '\n')
        ...
        #----- Position有り ----------
            ...
            if(F_LONG3rd > 0.0):
                if(d_profit > 0):
                    ...
                    s_dsp = "232 d_profit3rd:%d = (d_profit:%d ＊L_3rd:%f )" % (d_profit3rd, d_profit, F_LONG3rd)
                    print(s_dsp)
            ...
            #     s_dsp = "232 Down With Position D_TopBidPrice:%d d_profit:%d d_profit3rd:%d D_TargetPrice:%d d_Target:%d" % (D_TopBidPrice, d_profit, d_profit3rd, D_TargetPrice, d_Target)
            #     print(s_dsp)
            #     #     s_dsp = "232 3rd[%] Target Price,,,,%d,%d,   %d,%d,   %d,%d,%d,   %d,%d,%f" % (D_Current, d_currentDiff, D_BidPrice, d_profit, d_TopLast, D_TopBidPrice,  d_BottomPrice, d_LosscutPrice, D_TargetPrice, F_LONG3rd)
            #     #     print(s_dsp)
            #     if(F_Write):
            ...
            # 2nd Target-Price
                ...
                if(D_TopBidPrice < (D_BidPrice + D_LONG2nd)):
            ...
            f_Stop = False
            # (Profit有り・強制終了:LONGCLOSE = 1)
            if(D_LONGCLOSE > 0):
                f_Stop = True
                s_dsp = "- 240 CLOSE,%s,,%s,%d,%d,   %d,%d,   %d,%d,%d,   %d,%d" % (Symbol, s_Time, D_Current, d_currentDiff, D_BidPrice, d_profit, d_TopLast, D_TopBidPrice,  d_BottomPrice, d_LosscutPrice, D_TargetPrice)
            # Target-Price
            ...
            # 処分売り
            # if(D_Current <= d_LosscutPrice or D_Current < D_BidPrice  or D_LONGCLOSE > 0):
            if(f_Stop or D_LONGCLOSE > 0):
                if(d_profit > 0):
                    s_dsp = "- 241 Profit+,%s,,%s,%d,%d,   %d,%d,   %d,%d,%d,   %d,%d" % (Symbol, s_Time, D_Current, d_currentDiff, D_BidPrice, d_profit, d_TopLast, D_TopBidPrice,  d_BottomPrice, d_LosscutPrice, D_TargetPrice)
                else:
                    s_dsp = "- 241 Profit-,%s,,%s,%d,%d,   %d,%d,   %d,%d,%d,   %d,%d" % (Symbol, s_Time, D_Current, d_currentDiff, D_BidPrice, d_profit, d_TopLast, D_TopBidPrice,  d_BottomPrice, d_LosscutPrice, D_TargetPrice)
                print(s_dsp)
                if(F_Write):
