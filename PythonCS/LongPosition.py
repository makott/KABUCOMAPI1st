...
# LONGEVENT = WebsocketLong.LONGEVENT
# LONG0 = WebsocketLong.LONG0
# LONG1st = WebsocketLong.LONG1st
# LONG2nd = WebsocketLong.LONG2nd
# LONG3rd = WebsocketLong.LONG3rd
# LONGLOSS = WebsocketLong.LONGLOSS
# LONGCLOSE = WebsocketLong.LONGCLOSE
#------------------------------------------------------------------------------
#							＊
...
#----------------------------------------------------------
#----- 株価が変わった場合に呼ばれる関数 ----------------------
def Ttrading_Long(Symbol, CurrentPrice):
    # global Password
    # global Token
    # global Issue1
    ...
    LONGLOSS = WebsocketLong.LONGLOSS
    LONGCLOSE = WebsocketLong.LONGCLOSE
    d_LONGEVENT = WebsocketLong.d_LONGEVENT
    d_LONG0 = WebsocketLong.d_LONG0
    d_LONG1st = WebsocketLong.d_LONG1st
    d_LONG2nd = WebsocketLong.d_LONG2nd
    d_LONG3rd = WebsocketLong.d_LONG3rd
    d_LONGLOSS = WebsocketLong.d_LONGLOSS
    d_LONGCLOSE = WebsocketLong.d_LONGCLOSE

    d_LastValue = WebsocketLong.d_LastValue
    d_BidPrice = WebsocketLong.d_BidPrice
    d_TopPrice = WebsocketLong.d_TopPrice
    d_BottomPrice = WebsocketLong.d_BottomPrice
    d_LosscutPrice  = WebsocketLong.d_LosscutPrice
    d_TargetPrice = WebsocketLong.d_TargetPrice

    t_BidTime  = WebsocketLong.t_BidTime
    d_TopLast = WebsocketLong.d_TopLast
#  d_LastValue, d_CurrentPrice, d_Diff, d_LONG1st, d_BottomPrice, 
 
    Issue1 = WebsocketLong.Issue1
    #----------------------------------------------------------
    t_now = datetime.datetime.now().time()
    s_time = t_now.strftime("%H:%M:%S:%f")
    s_BidTime = t_BidTime.strftime("%H:%M:%S:%f")
    s_dsp = "  %s, %s,--- Issue1:%s ---,LONGEVENT:%d,LONG0:%d,LONG1st:%d,LONG2nd:%d,LONG3rd:%d) " % (s_time,s_BidTime, Issue1,d_LONGEVENT,d_LONG0,d_LONG1st,d_LONG2nd,d_LONG3rd)
    print(s_dsp)
    #------------------------------------------------------
    flagLONG2nd = True
    if((d_LONGEVENT & 2)==0):
...
#         'Symbol': Issue1,        #銘柄          'Symbol': '6521',       #6521：オキサイド
#         'Exchange': 1,          #1:東証
#         'SecurityType': 1,      #1:株式
#         'Side': '2',            #2:買
...
#     #         s_dsp = "----- Result:%s/OrderId:%s  (BidPrice:%6d,Issue1:%s, d_LastValue:%6d) -----" % (content["Result"], content["OrderId"], BidPrice, Issue1, d_LastValue)
#     #         print(s_dsp)
...
# with open ( ‘csvファイル’ , ‘r’ ) as f :                #　’r’＝読み込み
#     reader = csv . reader ( f )
#     for line in reader :
#     print ( line )
...
#     reader = csv.reader(f)
#     line = [row for row in reader]      #行の要素を取得する
#     print ( line [ 1 ] [ 1 ] )
#     print('----------------------------------------------------------')
